...
FINDING = "The ancient versions all kept the cardinal — ἡμέρα μία, dies unus, yoma chad — and the modern English translations on both benches quietly convert it to the ordinal: the WEB prints 'the first day', Etheridge's targum prints 'the First Day', and Silbermann's Rashi glosses the lemma 'THE FIRST DAY (literally, one day)' in the very note that exists to ask why it is not 'first'. The question the whole crux is made of survives only in the languages nobody now reads it in. Where the two benches do meet, they meet on the grammar: Bereshit Rabbah 3:9 objects that 'one, second, third' is not a proper series, and Bruno of Segni answers exactly that objection with the logic of relatives — first and second are correlatives, and with no second yet in being nothing could be called first. Both halves of that meeting have a head, and it is the same man. Philo of Alexandria, writing in Greek before either bench existed, gives the grammatical argument first: Moses does not even call it first, so that it should not be counted in along with the others. That is Bruno's logic of relatives a thousand years early and Bereshit Rabbah 3:9's objection to the series eleven hundred years early. And he gives a reason neither bench states — day not first but one, because of the monosis of the intelligible world, its solitariness, which has a monadic nature. Monosis is not unity and it is not primacy; it is the condition of having nothing beside you, which is exactly what Rashi means by yachid be-olamo, that God was alone in his world that day because there was not yet a second anything to count with. Three benches read the cardinal as a claim about being alone rather than about position in a series, and the oldest of the three is the one neither of the others knew it had. And they part on the calendar: Ben Zoma derives from yom echad that the day follows the night, while Augustine states in so many words that the days are counted a mane usque in mane."
